...
def create_session(ssh_tunnel: dict, database: dict) -> sessionmaker:
    ...
    if ssh_address_or_host and ssh_username and ssh_password and SSH_TUNNEL_AVAILABLE:
        ...
        # 如果SSH tunnel不可用，直接連接資料庫
        if not SSH_TUNNEL_AVAILABLE:
            print("Warning: sshtunnel not available, using direct database connection")
        local_host = db_host
        local_port = db_port
